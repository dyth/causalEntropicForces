...
def entropic_forcing(logProbs, environment, forces):
    ...
    return nom * environment.TIMESTEP ** 2.0 / denom 


def entropic_forcing2(logProbs, environment, forces):
    'calculate the path integral and determine the next best move'
    pathIntegral = sum([np.multiply(f[i], lP) for i, lP in enumerate(logProbs)])
    nom = 2.0 * environment.TC * pathIntegral * environment.TIMESTEP ** 2
    denom = environment.TR * (len(forces) + 1) * environment.MASS
    return nom / denom
...
while True:
    ...
    move = entropic_forcing2(logProbs, environment, f)
    pos += move
...
# TODO: new method of computing entropic forces
# TODO: fix motion from force
